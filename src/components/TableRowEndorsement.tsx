...
const TableRowEndorsement = ({
  ...
  return (
    <Table.Row className="dark:bg-gray-800">
      ...
      <Table.Cell>{endorsement.mintUrl}</Table.Cell>
      <Table.Cell>
        {
          endorsement.rating ? (
            <Rating>
              {Array.from({length: 5}).map((_, i) => <Rating.Star key={i} filled={i < endorsement.rating!}/>)}
            </Rating>
          ) : 
          "N/A"
        }
      ...
      <Table.Cell>
        {user.pubkey === endorsement.userPubkey && (
          ...
          <TrashIcon onClick={handleDelete} className="h-6 w-6 text-red-600 hover:cursor-pointer" />
          </Tooltip>
        )}
      </Table.Cell>
    </Table.Row>
